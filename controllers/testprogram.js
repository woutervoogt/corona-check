      ...
      if (err) {
        ...
        for (let i = 0; i < foundData.length; i++) {
          ...
          YTData.updateOne(
            { _id: foundData[i]._id },
            {
              videoDescription: yTRes.data.items[0].snippet.description,
              viewCount: yTRes.data.items[0].statistics.viewCount,
            },
...
async function saveToDatabase(data) {
  ...
  for (i = 0; i < data.items.length; i++) {
    ...
    const myLoopPromise = await YTData.create(newYTData, function (
      err,
      newlyCreated
    ) {
      if (err) {
